    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            case MotionEvent.ACTION_DOWN: {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    if (allowSwipe && rect.contains(x, y)) {
                        setParentView(child);
                        setFrontView(child.findViewById(swipeFrontView), swipeListView);
            ...
            case MotionEvent.ACTION_UP: {
                ...
                generateAnimate(frontView, false, swapRight, downPosition);
//                if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE) {
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeListView.SWIPE_ACTION_NONE && velocityY < velocityX) {
                    ...
                    if (opened.get(downPosition)) {
                        ...
                        swipeListView.onStartOpen(downPosition, swipeCurrentAction, swipingRight, parentView);
                    }
    ...
    public void move(float deltaX) {
        swipeListView.onMove(downPosition, deltaX, parentView);
        ...
        float newDeltaX = Math.max(deltaX, 0);

        if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_DISMISS) {
            setTranslationX(parentView, newDeltaX);
            setAlpha(parentView, Math.max(0f, Math.min(1f,
                    1f - 2f * Math.abs(newDeltaX) / viewWidth)));
        } else if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE) {
            if ((swipingRight && newDeltaX > 0 && posX < DISPLACE_CHOICE)
                    || (!swipingRight && newDeltaX < 0 && posX > -DISPLACE_CHOICE)
                    || (swipingRight && newDeltaX < DISPLACE_CHOICE)
                    || (!swipingRight && newDeltaX > -DISPLACE_CHOICE)) {
                setTranslationX(frontView, newDeltaX);
            }
        } else {
            setTranslationX(frontView, newDeltaX);
        }
