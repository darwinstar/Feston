...
public class SwipeListView extends ListView {
		...
		if (swipeFrontView == 0 || swipeBackView == 0) {
			...
			if (swipeFrontView == 0 || swipeBackView == 0) {
				throw new RuntimeException(String.format(
						"You forgot the attributes swipeFrontView or swipeBackView. You can setEvents this attributes or use '%s' and '%s' identifiers", SWIPE_DEFAULT_FRONT_VIEW,
	 ...
	 *            to right
	 */
	public void onStartOpen(int position, int action, boolean right, View view) {
		if (swipeListViewListener != null && position != ListView.INVALID_POSITION) {
			swipeListViewListener.onStart(position, action, right, view);
		}
	}
	...
	public void onClickFrontView(int position, AdapterView adapterView) {
		if (swipeListViewListener != null && position != ListView.INVALID_POSITION) {
			swipeListViewListener.onClickFrontView(position, adapterView);
		}
	}
	...
	public void onMove(int position, float x, View backView) {
		if (swipeListViewListener != null && position != ListView.INVALID_POSITION) {
			swipeListViewListener.onMove(position, x, backView);
		}
	}

	public void onAnimationEnd(View parentView) {
		if (swipeListViewListener != null) {
			swipeListViewListener.onEnd(parentView);
		}
	}
