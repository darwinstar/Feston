...
public class Artist_details extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		((Artist_details_container) getActivity()).getSupportActionBar().setDisplayHomeAsUpEnabled(true);
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		// artist = (Artists) getArguments().getSerializable("Artist");
		...
		tvTitle.setText(((Artist_details_container) getActivity()).artist.getTitle());
		tvCompany.setText(((Artist_details_container) getActivity()).artist.getCompany());
		if (((Artist_details_container) getActivity()).artist.getDescription_en() != null) {
			tvDescription.setText(Html.fromHtml(((Artist_details_container) getActivity()).artist.getDescription_en()));
		}
		Picasso.with(activity).load(((Artist_details_container) getActivity()).artist.getImage()).into(ivImage, new Callback() {
			@Override
			...
			@Override
			public void onError() {
				Log.e("ERROR", "ERROR LOADING___" + ((Artist_details_container) getActivity()).artist.getTitle() + "___");
	...
	private void ToDatabase(int select) {
		((Artist_details_container) getActivity()).artist.setSelected(select);
		...
		Log.e("TAGAS", ((Artist_details_container) getActivity()).artist.getArtistId() + " artist id");
		sql.update(Config.TABLE_ARTISTS, cv, "artistid=?", new String[] { ((Artist_details_container) getActivity()).artist.getArtistId() });
	...
	private void SelectedOrNot() {
		if (((Artist_details_container) getActivity()).artist.getSelected() == 1) {
			tick.setVisible(false);
			cross.setVisible(true);
		} else if (((Artist_details_container) getActivity()).artist.getSelected() == 0) {
			tick.setVisible(true);
			cross.setVisible(false);
		}
	}
